...
#checkpointDir = "file:///tmp/streaming/write_kafka_final"
...
#        .withColumn("event_ts_min", from_unixtime(unix_timestamp(F.split("value", ",")[8], "yyyy-MM-dd HH:mm:ss"))) \
#        .withColumn("event_ts_min", F.split(F.col("value"), ",")[8].cast(StringType())) \
...
def check_if_index_exists(es):
 ...
 #       logging.info("Index office_input already exists")
        #es.indices.delete("online-retail-spark")
        #print("online-retail-spark  index deleted.")
        #es.indices.create(index="office_input", body=office_input_index)
        #print("Index office_input created again")
#    else:
